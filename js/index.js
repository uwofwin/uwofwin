...
$(document).ready(function() {
    ...
    function getUserProfit() {
        $(".p-gen").html((player[21] / 1000000).toFixed(2));
        $(".p-dy").html((player[22] / 1000000).toFixed(2));
        $(".p-win").html((player[23] / 1000000).toFixed(2));
            ...
            if (player[24] == 0) {
                if (player[11] <= fuseTime) {
                    ...
                    if (diffTime >= oneDay) {
                        ...
                        if (unIncomeDays > 0) {
                            unIncome1 = player[4] / 1000000 * 0.3 / 100 * unIncomeDays;
                            unIncome2 = player[4] / 1000000 * 2.2 / 100 * unIncomeDays;
                        }
                    }
        ...
        let temp;
        if (usdtConvert(200) <= player[6] && player[6] <= usdtConvert(2000)) {
            temp = 3;
        } else if (usdtConvert(2001) <= player[6] && player[6] <= usdtConvert(6000)) {
            temp = 4;
        ...
        $(".pext-incomecan").html((Number((player[12]) - Number(player[5])) / 1000000).toFixed(0));

        $(".node-gen").html(((Number(player[21]) + Number(player[22])) / 1000000).toFixed(2));

        $(".p-unwithdraw").html((player[17] / 1000000).toFixed(2));

        $(".p-teth").html((player[6] / 1000000).toFixed(2));

        $(".p-tstc").html((player[16] / 1000000).toFixed(2));

        $(".p-tdy").html((player[9] / 1000000).toFixed(2));

        $(".pext-teamgen").html((player[7] / 1000000).toFixed(2));

        $(".pext-anglegen").html((player[8] / 1000000).toFixed(2));


        if (smallPoolPrize[5] == 1) {
            $(".s-pot").html("0.00");
        } else {
            $(".s-pot").html((smallPoolPrize[1] / 1000000).toFixed(2));
        }
        ...
        if (bigPoolPrize[5] == 1) {
            ...
            $(".b-pot").html((bigPoolPrize[1] / 1000000).toFixed(2));
        }
    }

    function checkFusingState(bppNum) {
        fuseTime = bigPoolPrize[7];
    ...
    function calPCC() {
        ...
        var ethinputVal_ = parseInt(ethinputVal * 1000000 / pccToUsdtRate)
        $("#exchangePCC").html((ethinputVal_ / 10000000));
    }
    ...
    $(".buywitheth").click(async function() {
        ...
        var usdtEtherValue = usdtEtherValue1 * 1000000;
        ...
        let pcc = parseInt(usdtEtherValue / rate);
        pcc = (pcc / 10000000);
    ...
    $("#buy-ticket-action").click(async function() {
        ...
        try {
            ...
            var usdtEtherValue = $(".ticket-stsinput").html();
            usdtEtherValue = usdtEtherValue * 1000000;
        ...
        for (let i = 0; i < idArr.length; i++) {
            await ticketsInstance.methods.getReferrerById(idArr[i]).call().then(function(res) {
                ...
                html += "<div class=\"col-md-12 col-xs-12 col-sm-12 col-md-12\" style=\"display: flex;flex-flow: nowrap;justify-content: space-between;align-items: center;line-height: 30px;\">" +
                    ...
                    "<div class=\"col-xs-6 text-right\">" + (amountArr[i] / 1000000).toFixed(0) + "</div>" +
                    "</div>";
            });
        }
        $('#minArrays').html("");
...
function getBigPoolList() {
    dividendsInstance.methods.getSmallPlayer(2).call().then(async function(res) {
        ...
        for (let n = startIndex + 1; n < idArr.length; n++) {
            if (maxAmt / 1000000 < amountArr[n] / 1000000) {
                maxAmt = amountArr[n];
                minTime = timeList[n];
                maxIndex = n;
            } else if (maxAmt / 1000000 == amountArr[n] / 1000000) {
        ...
        await dividendsInstance.methods.getAddrById(idArr[maxIndex]).call().then(function(res) {
            ...
            html += "<div class=\"col-md-12 col-xs-12 col-sm-12 col-md-12\" style=\"display: flex;flex-flow: nowrap;justify-content: space-between;align-items: center;line-height: 30px;\">" +
                "<div class=\"col-xs-6 text-left\">" + addr + "</div>" +
                "<div class=\"col-xs-6 text-right\">" + (maxAmt / 1000000).toFixed(0) + "</div>" +
                "</div>";
        });
        $('#maxArrays').html("");
